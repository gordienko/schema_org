...
RSpec.describe SchemaOrg::SearchAction do
  describe '#initialize' do
    it 'sets the default @type to SearchAction' do
      search_action = SchemaOrg::SearchAction.new('query-input': 'test query', target: 'https://example.com/search')
    ...
    it 'assigns all required attributes correctly' do
      ...
      expect(search_action.to_h[:'query-input']).to eq(attributes[:'query-input'])
      expect(search_action.to_h[:target]).to eq(attributes[:target])
    end

    it 'raises an error if query-input is missing' do
      expect { SchemaOrg::SearchAction.new(target: 'https://example.com/search') }
        .to raise_error(Dry::Struct::Error, /:"query-input" is missing in Hash input/)
    end

    it 'raises an error if target is missing' do
      expect { SchemaOrg::SearchAction.new('query-input': 'test query') }
